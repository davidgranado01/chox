...
<head>
    ...
    <script type="text/javascript">
        ...
        <s:if test="isChoxAdmin!=true">
            ...
            function cancelExport(btn){
                if (btn == 'cancel'){
                    Ext.MessageBox.hide();
                    exportIntervelId=window.clearInterval(exportIntervelId);
                    Ext.Ajax.request({
                        url:'<%= request.getContextPath()%>/prv/p/cancelExport.action',
                        callback : function(options,success,response  ){
            ...
            var loadLiveExportClaimCount = function updateExportedClaim(){
                
                Ext.Ajax.request({
                    url:'<%= request.getContextPath()%>/prv/p/updateExportClaimsCount.action',
                    callback : function(options,success,response  ){
                        if(response.responseText){
                            var resp = Ext.util.JSON.decode(response.responseText);
