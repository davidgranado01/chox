...
<script type="text/javascript">
    ...
    Ext.onReady(function(){
       ...
       gridviewJsonReader = new Ext.data.JsonReader({
        ...
        gridviewData = new Ext.data.Store({
            ...
            ({url: '<%= request.getContextPath()%>/prv/p/getInsurer.action',method:'GET'}),
    ...
    function loadSelectedRecord(grid, rowIndex, columnIndex, e){
        ...
        $("#admin_param_panel").load("<%= request.getContextPath()%>/prv/p/updateInsurerDetailPanel.action?objectId=" + gridViewId+uniqeToken());
    }
    ...
    function triggerStatusUpdateRecord(gridView){
        ...
        if(deleteAtt){
            var gridViewId = gridView.get("id");
            $.ajax({
                url: "<%= request.getContextPath()%>/prv/p/doTriggerInsurerAccountStatus.action?objectId="+gridViewId+uniqeToken(),
