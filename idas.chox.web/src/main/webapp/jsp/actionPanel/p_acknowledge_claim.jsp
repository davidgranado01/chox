...
<s:if test="copleyOfferEnabled">
                            ...
                            <tr>
                                <td colspan="4" class="choice" nowrap>
                                    <input type="button" id="ACRejectButtonId" value="Reject" <s:if test="rejectButtonEnabled == false">disabled='true'</s:if> onclick="event.preventDefault(); doAcknowledgeFormSubmit('rejectClaim');" />
                                    <input type="button" id="ACAcknowledgeButtonId" value="Acknowledge" onclick="event.preventDefault(); doAcknowledgeFormSubmit('acknowledgeClaim');"  />
                                    <s:if test="insurerIsEngineersEnabled">
                                        <input type="button" id="ACReferToEngineerButtonId" value="Refer To Engineer" onclick="event.preventDefault(); doAcknowledgeFormSubmit('referEng');" />
                                    </s:if>
                                    <s:if test="insurerIsFnolEnabled && !updatedByEng">
                                        <input type="button" id="ACReferToFnolButtonId" value="Refer to FNOL" onclick="event.preventDefault(); doAcknowledgeFormSubmit('referFNOL');" />
                                    </s:if>
                                    <input type="button" id="ACClaimPendingButtonId" value="Claim Pending" onclick="event.preventDefault(); doAcknowledgeFormSubmit('pending');" />
                                </td>
                            </tr>
                        </table>
