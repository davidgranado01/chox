...
public class ClaimAction extends BaseAction implements ModelDriven<Claim>, Preparable {
    ...
    public Integer getAuditReviewClaimType() {
        if (claim.getInvoice().getHireGross().compareTo(BigDecimal.ZERO) == 1 
            ...
            return AuditReviewClaimType.REPAIR_ONLY.getValue(); 
        } else {
            return AuditReviewClaimType.OTHER.getValue(); 
        }
    }

    public Integer getAuditReviewClaimTypeFromModel() {
        if (claim.getClaimAuditReview() != null) {
            for (AuditReviewClaimType auditReviewClaimType : AuditReviewClaimType.values()) {
                if (auditReviewClaimType.getDescription().equals(claim.getClaimAuditReview().getClaimType())) {
                    return auditReviewClaimType.getValue();
                }
            }
        }
        return null;
    ...
    // The below method used to decide whether to show the default value on the UI forms or not.
    public boolean getClaimAuditReviewSaved() {
        return claim.getClaimAuditReview() != null 
                && claim.getClaimAuditReview().getLastModifiedDate().compareTo(claim.getClaimAuditReview().getCreatedDate()) != 0;
    }
    ...
    public String getClaimAuditPanel() {
        return SUCCESS;
    }
    ...
    public String updateSaveLiabilityStatus() {
        ...
        try {
            if (claim.getLiabilityStatus() == LiabilityStatus.LIABILITY_NULL || !claim.getLiabilityStatus().equals(fLiabilityStatus)) {
                ...
                claimService.updateLiabilityPercentages(claim, fPercentageLiabilityAccepted, fPercentageLiabilityCho);
    ...
    public String getActionPanel() {
        ...
        for (String action : actions) {
            ...
            if (accessRight > 0 && ("InvoicePaymentLogged".equals(action) || "UpdateManualInvoicePaid".equals(action))
                ...
                accessRight = 0;
            } else if (accessRight > 0 && action.equals(ActionPanel.CLAIM_ADUIT)) {
                accessRight = claim.getClaimAuditReview() != null ? claim.getClaimAuditReview().isClaimAuditReviewCompleted() ? 0 : accessRight : 0;
            }
            if (accessRight >= 2) {
            ...
            if (accessRight >= 2) {
                switch (actionName) {
                    ...
                    case ExtraAction.MARK_SUPPLEMENTARY_INVOICED_CLAIM:
                        ...
                        break;
                    case ExtraAction.REVIEW_CLAIM_ADUIT:
...
//    }
    ...
    public String getFormattedLiability() {
        if (claim.getPercentageLiabilityAccepted().compareTo(claim.getAppliedLiability()) != 0) {
            return String.format("Insurer %s%% (Applied %s%%)", claim.getPercentageLiabilityAccepted() == null ? BigDecimal.ZERO.setScale(2).toPlainString() : claim.getPercentageLiabilityAccepted().toPlainString(),
                claim.getAppliedLiability() == null ? BigDecimal.ZERO.setScale(2).toPlainString() : claim.getAppliedLiability().toPlainString());
        } else {
...
//        jObject = JSONArray.fromObject(viewDatas);
    ...
    public BigDecimal getPaymentDetailsCHODiscount() {
        if (claim.getInvoice() != null) {
            if (ClaimType.isInsurerVsInsurer(claim.getClaimType()) || ClaimType.isSubscriber(claim.getClaimType())
                ...
                return claim.getInvoice().getDiscount().multiply(claim.getAppliedLiability()).divide(new BigDecimal(100), 2, BigDecimal.ROUND_HALF_UP);
            }
        } else {
    ...
    public BigDecimal getPaymentDetailsClaimHandInvAmt() {
        if (claim.getInvoice() != null) {
            if (ClaimType.isInsurerVsInsurer(claim.getClaimType()) || ClaimType.isSubscriber(claim.getClaimType())
                ...
                return claim.getInvoice().getClaimsHandlingInvoiceAmount().multiply(claim.getAppliedLiability()).divide(new BigDecimal(100), 2, BigDecimal.ROUND_HALF_UP);
            }
        } else {
            return BigDecimal.ZERO;
        }
    ...
    public BigDecimal getPaymentDetailsDeductionClaimHandFee() {
        if (claim.getInvoice() != null) {
            if (ClaimType.isInsurerVsInsurer(claim.getClaimType()) || ClaimType.isSubscriber(claim.getClaimType())
                ...
                return claim.getInvoice().getDeductionForClaimsHandlingFee().multiply(claim.getAppliedLiability()).divide(new BigDecimal(100), 2, BigDecimal.ROUND_HALF_UP);
            }
        } else {
            return BigDecimal.ZERO;
        }
    ...
    public BigDecimal getPaymentDetailsInsurerDiscount() {
        if (claim.getInvoice() != null) {
            if (ClaimType.isInsurerVsInsurer(claim.getClaimType()) || ClaimType.isSubscriber(claim.getClaimType())
                ...
                return claim.getInvoice().getInsurerDiscount().multiply(claim.getAppliedLiability()).divide(new BigDecimal(100), 2, BigDecimal.ROUND_HALF_UP);
            }
        } else {
            return BigDecimal.ZERO;
        }
    ...
    public BigDecimal getEngineerFeeGrossPaid() {
        if (claim.getInvoice() != null) {
            if (ClaimType.isInsurerVsInsurer(claim.getClaimType()) || ClaimType.isSubscriber(claim.getClaimType())
                ...
                return claim.getInvoice().getEngineerFeeGross().multiply(claim.getAppliedLiability()).divide(new BigDecimal(100), 2, BigDecimal.ROUND_HALF_UP);
            }
        } else {
            return BigDecimal.ZERO;
        }
    ...
    public BigDecimal getHireGrossPaid() {
        if (claim.getInvoice() != null) {
            if (ClaimType.isInsurerVsInsurer(claim.getClaimType()) || ClaimType.isSubscriber(claim.getClaimType())
                ...
                return claim.getInvoice().getHireGross().add(claim.getInvoice().getGtaDiscount()).multiply(claim.getAppliedLiability()).divide(new BigDecimal(100), 2, BigDecimal.ROUND_HALF_UP);
            }
        } else {
            return BigDecimal.ZERO;
        }
    ...
    public BigDecimal getHirePenaltyChargePaid() {
        if (claim.getInvoice() != null) {
            if (ClaimType.isInsurerVsInsurer(claim.getClaimType()) || ClaimType.isSubscriber(claim.getClaimType())
                ...
                return claim.getInvoice().getHirePenaltyCharge().multiply(claim.getAppliedLiability()).divide(new BigDecimal(100), 2, BigDecimal.ROUND_HALF_UP);
            }
        } else {
            return BigDecimal.ZERO;
        }
    ...
    // The below value is used in claim audit review panel. 
    // sometimes totalling (getHirePenaltyChargePaid() + getRepairPenaltyChargePaid()) gives a penny out on totalPenaltyCharge.
    // To avoid that apply the liability on totalPenaltyCharge to calculate the totalPenaltyChargePaid amount.
    // For e.g see claim 394433/Papa . 
    public BigDecimal getTotalPenaltyChargePaid() {
        if (claim.getInvoice() != null) {
            if (ClaimType.isInsurerVsInsurer(claim.getClaimType()) || ClaimType.isSubscriber(claim.getClaimType())
                    || ClaimType.isFixedFee(claim.getClaimType()) || ClaimType.isCollaborationProtocol(claim.getClaimType())) {
                return claim.getInvoice().getTotalPenaltyCharge();
            } else {
                return claim.getInvoice().getTotalPenaltyCharge().multiply(claim.getAppliedLiability()).divide(new BigDecimal(100), 2, BigDecimal.ROUND_HALF_UP);
            }
        } else {
            return BigDecimal.ZERO;
        }
    ...
    public BigDecimal getRepairGrossPaid() {
        if (claim.getInvoice() != null) {
            if (ClaimType.isInsurerVsInsurer(claim.getClaimType()) || ClaimType.isSubscriber(claim.getClaimType())
                ...
                return claim.getInvoice().getRepairGross().multiply(claim.getAppliedLiability()).divide(new BigDecimal(100), 2, BigDecimal.ROUND_HALF_UP);
            }
        } else {
            return BigDecimal.ZERO;
        }
    ...
    public BigDecimal getRepairPenaltyChargePaid() {
        if (claim.getInvoice() != null) {
            if (ClaimType.isInsurerVsInsurer(claim.getClaimType()) || ClaimType.isSubscriber(claim.getClaimType())
                ...
                return claim.getInvoice().getRepairPenaltyCharge().multiply(claim.getAppliedLiability()).divide(new BigDecimal(100), 2, BigDecimal.ROUND_HALF_UP);
            }
        } else {
            return BigDecimal.ZERO;
        }
    ...
    public BigDecimal getStorageRecoveryGrossPaid() {
        if (claim.getInvoice() != null) {
            if (ClaimType.isInsurerVsInsurer(claim.getClaimType()) || ClaimType.isSubscriber(claim.getClaimType())
                ...
                return claim.getInvoice().getStorageRecoveryGross().multiply(claim.getAppliedLiability()).divide(new BigDecimal(100), 2, BigDecimal.ROUND_HALF_UP);
            }
        } else {
            return BigDecimal.ZERO;
        }
    ...
    public BigDecimal getTotalLossFeeGrossPaid() {
        if (claim.getInvoice() != null) {
            if (ClaimType.isInsurerVsInsurer(claim.getClaimType()) || ClaimType.isSubscriber(claim.getClaimType())
                ...
                return claim.getInvoice().getTotalLossFeeGross().multiply(claim.getAppliedLiability()).divide(new BigDecimal(100), 2, BigDecimal.ROUND_HALF_UP);
            }
        } else {
            return BigDecimal.ZERO;
        }
