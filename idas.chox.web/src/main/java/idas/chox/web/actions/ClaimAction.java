...
//                }
    ...
    public String getRepairPenaltyPercentageJsonString() {
        List<LookupItem> luItems = new ArrayList<LookupItem>(PenaltyPercentage.getRepairPenaltyPercentage().size());
        for (PenaltyPercentage repairPenaltyPercentageEnum : PenaltyPercentage.getRepairPenaltyPercentage()) {
...
//                setActionResult("The 'Penalty Charge Calculation Date' cannot be set to before the invoice was uploaded. Your changes have not been saved.");
    ...
    private boolean isEscalatedToSupervisor(){
        if(service.getDaysSinceInvoiceUploadToEscalate(claim.getId()) >= getAuthenticatedUser().getInsurer().getDaysBeforeEscalated()
                || service.getNumberOfTimesContestedWithCHOtoEscalate(claim.getId()) >=  getAuthenticatedUser().getInsurer().getTimesInStatusContested()){
            return true;
        }
        return false;
    }
