        ...
        if (filename.exists()) {
            ...
            try {
                ...
                while ((choReference = br.readLine()) != null) {
                    reopenClaim(uploadService, choReference);
                }
            ...
            reopenClaim(uploadService, file);
        }

    }

    public static void reopenClaim(UploadService uploadService, String choReference) {
        Result result = null;

        LOG.debug("Calling reopenClaim Web Service for claim with CHO reference '{}'...", choReference);
        try {
            result = uploadService.reopenClaim(choReference);
        } catch (Exception ex) {
            LOG.error("Error calling reopenClaim web service: {}", ex.getMessage());
            if (ex.getCause() != null) {
        ...
        if (!result.isStatus()) {
            LOG.error("Error re-opening claim '{}' : {}", choReference, result.getErrorMessage());
        } else {
            LOG.info("Claim with CHO reference '{}' has been re-opened.", choReference);
        }
    }
}
