...
SELECT 'dl-ukbr-audatex-penguin-reporting@audatex.com', 'Paid Invoices', '(via SFTP)', '{bcrypt}$2a$10$tOlpJVr.Q6TYCKKv8/LgHOaGRQWxgwMkZKP8q.zWGgMeqpt3yvPfS', 999, NOW(), 999, NOW(),
  qbe_id, 't', 'f', new_user_name, 0, 'f', NOW(), 'f', 0, 'f'
WHERE NOT EXISTS (SELECT * FROM web_user WHERE user_name = new_user_name)
...
SELECT new_user_id, role_ins_id, 999, NOW(), 999, NOW(), 't', 0
WHERE NOT EXISTS (SELECT * FROM web_user_user_role WHERE web_user_id = new_user_id AND web_user_role_id = role_ins_id);

INSERT INTO web_user_user_role (web_user_id, web_user_role_id, created_by, created_date, last_modified_by, last_modified_date, is_active, version)
SELECT new_user_id, role_ins_ch_id, 999, NOW(), 999, NOW(), 't', 0
WHERE NOT EXISTS (SELECT * FROM web_user_user_role WHERE web_user_id = new_user_id AND web_user_role_id = role_ins_ch_id);
...
SELECT new_user_name, '', 'PAID_INVOICES', 't', 0, 999, NOW(), 999, NOW()
WHERE NOT EXISTS (SELECT * FROM scheduler_job WHERE login_username = new_user_name AND job_name = 'PAID_INVOICES')
