...
public class InsurerCountryLocation {
    ...
    public static InsurerCountryLocation getCatchAllForRental(DBConnectionWrapper connection,Rental rental) throws SQLException
    {
    ...
    public static InsurerCountryLocation instantiate(DBConnectionWrapper connection, long id) throws SQLException {
        String cacheKey = "chox.data.InsurerCountryLocation.instantiate";
        try {
            return (InsurerCountryLocation) Cache.getInstance().get(cacheKey, id);
        } catch (CacheMissException e) {
            PreparedStatement s = connection.prepareStatement("select insurer_country_id,name,catch_all from insurer_country_location where id=?");
            s.setLong(1, id);
            InsurerCountryLocation o = null;
            ResultSet rs = s.executeQuery();
            if (rs.next()) {
                o = new InsurerCountryLocation();
    ...
    public void save(DBConnectionWrapper connection) throws SQLException {
        if (getID() == -1) {
            PreparedStatement s = connection.prepareStatement("insert into insurer_country_location(insurer_country_id,name,catch_all) values (?,?,?) returning id");
            ...
            PreparedStatement s = connection.prepareStatement("update insurer_country_location set insurer_country_id=?,name=?,catch_all=? where id=?");
            s.setLong(1,getInsurerCountryID());
            s.setString(2,getName());
            s.setString(3,getCatchAll());
            s.setLong(4, getID());
