...
public class EngineerCost {
    ...
    public static EngineerCost getForClaim(DBConnectionWrapper connection,Claim claim) throws SQLException {
        EngineerCost c=null;        
        PreparedStatement s=connection.prepareStatement("select id from engineer_cost where claim_id=?");
    ...
    public static void removeForClaim(DBConnectionWrapper connection, Claim claim) throws SQLException {
        PreparedStatement s = connection.prepareStatement("delete from engineer_cost where claim_id=?");
        s.setLong(1, claim.getID());
        s.executeUpdate();
        s.close();
    }

    public static EngineerCost instantiate(DBConnectionWrapper connection, long id) throws SQLException {
        String cacheKey = "chox.data.EngineerCost.instantiate";
        try {
            return (EngineerCost) Cache.getInstance().get(cacheKey, id);
        } catch (CacheMissException e) {
            PreparedStatement s = connection.prepareStatement("select claim_id,net_fee,fee_vat,gross_fee from engineer_cost where id=?");
            s.setLong(1, id);
            EngineerCost o = null;
            ResultSet rs = s.executeQuery();
            if (rs.next()) {
    ...
    public void save(DBConnectionWrapper connection) throws SQLException {
        if (getID() == -1) {
            PreparedStatement s = connection.prepareStatement("insert into engineer_cost(claim_id,net_fee,fee_vat,gross_fee) values (?,?,?,?) returning id");
            ...
            PreparedStatement s = connection.prepareStatement("update engineer_cost set rental_id=?,net_fee=?,fee_vat=?,gross_fee=? where id=?");
            s.setLong(1, getClaimID());
            s.setBigDecimal(2, getNetFee());
            s.setBigDecimal(3, getFeeVAT());
            s.setBigDecimal(4, getGrossFee());
