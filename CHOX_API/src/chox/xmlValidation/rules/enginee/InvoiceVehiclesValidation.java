...
public class InvoiceVehiclesValidation extends SecureDataService implements rulesInterface{
    ...
    public InvoiceVehiclesValidation(
    ...
    public ClaimResult execute() throws DOMException, XPathExpressionException, Exception{
        ...
        this.element = XMLUtils.getElement(root, "vehicles");
    ...
    private boolean validate() throws DOMException, XPathExpressionException, Exception{
        ...
        if(claimResult.getClaimParseStatus().equals(ClaimParseStatus.newInvoice)){
            ...
            this.claimResult = NodeHelper.nodeValidate(sectionName, "day-rate", this.element, claimResult, dataValidationParameter);
    ...
    private void process(){
        ...
        this.claimResult.getClaim().getInvoice().setHireRateChargedPerDay(XmlHelper.getBigDecimalFromNode(this.element, "day-rate"));
    }
    ...
    private void doPrintResult(boolean isAllowed){    
        ...
        if(isAllowed){
            ...
            if(claimResult.getClaim().getInvoice()!=null){
                System.out.println(sectionName + "| getHireGross :"+claimResult.getClaim().getInvoice().getHireGross());
                System.out.println(sectionName + "| getHireNet :"+claimResult.getClaim().getInvoice().getHireNet());
                System.out.println(sectionName + "| getHireVat :"+claimResult.getClaim().getInvoice().getHireVat());
                System.out.println(sectionName + "| getHireRateChargedPerDay :"+claimResult.getClaim().getInvoice().getHireRateChargedPerDay());
            }else{
