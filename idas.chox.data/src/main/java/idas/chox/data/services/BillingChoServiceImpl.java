...
public class BillingChoServiceImpl extends SecureDataService implements BillingChoService{
	...
	/* (non-Javadoc)
	 * @see idas.chox.data.services.BillingChoService#checkObject(java.lang.String, java.util.Date, java.util.Date)
	 */
	public Map checkObject(String scheduleName,Date dateFrom,Date dateTo){
        ...
        DetachedCriteria dc2 = DetachedCriteria.forClass(BillingCho.class).add(Restrictions.ge("dateTo", dateFrom));       
        List result2 =  getHibernateTemplate().findByCriteria(dc2);
        if ( result2 != null && result2.size() > 0) {
        	for (Object object : result2) {
				log.debug(((BillingCho)object).getScheduleName() + " schedule name ");
			}
        	checks.put("dateFrom", "From date overlaps existing schedule.");
        }
        
        DetachedCriteria dc3 = DetachedCriteria.forClass(BillingCho.class).add(Restrictions.ge("dateTo", dateTo));       
