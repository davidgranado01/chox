    ...
    RETURN QUERY
        ...
        SELECT ''::varchar as "AX Unique Ref",
               ...
               case when exists(select * from audit_trail at where at.claim_id = c.id and at.new_status='AwaitingCarHireInfo'
                                                               and at.created_date >=  DATE_FROM and at.created_date < DATE_TO and reverted = false ) then 'CHOX'::varchar else
                        case when c.id = 6 then 'Birmingham'::varchar else case when c.id = 26 then 'Leeds'::varchar else ''::varchar end end end as "Branch / Scanning Centre",
               tp.policy_number as "Policy Number",
               removeCarriageReturns(c.claim_number) as "Claim Number",
               removeCarriageReturns(tp.title || ' ' || tp.first_name || ' ' || tp.last_name)::TEXT as "Policy Holder",
               removeCarriageReturns(tp.vehicle_registration) as "Customer Registration Number",
               inc.date as "Date of Accident",
               removeCarriageReturns(inc.incident_description)::TEXT as "Accident Circumstances",
               removeCarriageReturns(c.cho_reference) as "Hire Firm reference number",
               ...
               removeCarriageReturns(cust.address1 || ' ' || cust.address2  || ' ' || cust.address3 || ' '
                                         || cust.address4 || ' ' || cust.address5 || ' ' || cust.postcode)::TEXT as "Hire Firm Customer Address",
               removeCarriageReturns(hmd.name_of_repairer) as "TP Repairer Name",
               removeCarriageReturns(hmd.name_of_repairer) as "TP Repairer Phone Number",
               removeCarriageReturns(cust.location) as "Vehicle Location",
               removeCarriageReturns(cust.location) as "Vehicle Location Details",
               removeCarriageReturns(cust.vehicle_registration) as "Claimant Vehicle Registration",
               removeCarriageReturns(cust.damage) as "Vehicle Damage",
               ''::varchar as "Injury",
               vh.rental_start as "Hire Commenced On (If applicable)",
               removeCarriageReturns(tp.insurer_brand) as "RBS Insurance Brand",
               getLiabilityStatusAx(c.liability_status) as "Liability status",
               (select removeCarriageReturns(comment) from comment where claim_id=c.id and comment like 'Supporting Liability Notes%' order by id desc limit 1) as "If wrong insurer, please provide correct info",
