...
select 'avg_hire_value_paid' as title , (select avg(i.hire_gross)::numeric(8,2) from claim c , invoice i
   where c.invoice_id = i.id
...
(select avg(i.hire_gross)::numeric(8,2) from claim c , invoice i
where c.invoice_id = i.id
      and c.status = 'PaymentReceived'
      and (c.insurer_id = params.insurerId or params.insurerId = -1)                                        
      and (c.chorganisation_id = params.chorgId or params.chorgId = -1)
...
(select avg(i.hire_penalty_charge_paid + i.repair_penalty_charge_paid)::numeric(8,2) from claim c, invoice i, chorganisation cho
  where c.invoice_id=i.id  
       and c.status = 'PaymentReceived'
       and (c.insurer_id = params.insurerId or params.insurerId = -1)
       and (i.hire_penalty_charge_paid > 0.0 or i.repair_penalty_charge_paid > 0.0)
...
(select sum(i.hire_penalty_charge_paid + i.repair_penalty_charge_paid)::numeric(8,2) from claim c, invoice i, chorganisation cho
  where c.invoice_id=i.id  
       and c.status = 'PaymentReceived'
       and (c.insurer_id = params.insurerId or params.insurerId = -1)
       and (i.hire_penalty_charge_paid > 0.0 or i.repair_penalty_charge_paid > 0.0)
...
(select sum(i.hire_gross)::numeric(12,2) from claim c , invoice i, audit_trail a, chorganisation cho
    where c.invoice_id = i.id
      and a.claim_id = c.id
      and a.new_status = 'PaymentReceived' and c.status = 'PaymentReceived'
      and a.reverted = false
