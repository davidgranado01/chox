drop function dlg_pilot_report(text, integer, integer[], integer[], text, text);
-- select * from dlg_pilot_report('2016-04-16', 6, array[1007,1008,1009,1125], null, '2015-11-01', '2015-10-31');
create or replace function dlg_pilot_report
(
   reportStart text, insid integer, chorgs integer[], claimType integer[], claimUpload text, rental text
)
returns table
(
   id   integer,
   ...
   previous_6_month numeric(12,2),
   previous_7_month numeric(12,2)
)
as $$
...
(select count(*) from claim c left outer join vehicle_hire vh on (c.vehicle_hire_id = vh.id), invoice i
...
(select count(*)
 from claim c left outer join vehicle_hire vh on (c.vehicle_hire_id = vh.id), invoice i, chorganisation cho
    where c.invoice_id=i.id
       and c.created_date > claimUploadStart
       and (vh is null or vh.rental_start is null or vh.rental_start >= rentalStart)
       and (case when array_length(claimType, 1) > 0 then c.claim_type = ANY(claimType) else true end)
...
from claim c left outer join vehicle_hire vh on (c.vehicle_hire_id = vh.id), invoice i
    where c.invoice_id=i.id
       and c.created_date > claimUploadStart
       and (vh is null or vh.rental_start is null or vh.rental_start >= rentalStart)
...
(select coalesce(avg(o.hire_gross), 0)::numeric(12,2) from claim c left outer join vehicle_hire vh on (c.vehicle_hire_id = vh.id), invoice_original o, invoice i, chorganisation cho
...
(select coalesce(avg(case when vh.days_original is not null then vh.days_original else vh.days end), 0)::numeric(8,1) from claim c, invoice i, vehicle_hire vh, chorganisation cho
  where c.invoice_id=i.id and c.vehicle_hire_id = vh.id
       and c.created_date > claimUploadStart
       and vh.rental_start >= rentalStart
       and i.hire_net - i.admin_fee > 0
       and (case when array_length(claimType, 1) > 0 then c.claim_type = ANY(claimType) else true end) 
       and (c.insurer_id = params.insurerId or params.insurerId = -1)
       and c.chorganisation_id = cho.id and (params.chorgIds is null or c.chorganisation_id = ANY(params.chorgIds))
...
(select coalesce(sum(case when i.final_payment is not null then i.repair_gross_paid else i.repair_gross end), 0)::numeric(14,2) from claim c left outer join vehicle_hire vh on (c.vehicle_hire_id = vh.id), invoice i, chorganisation cho
     where c.invoice_id = i.id
       and c.created_date > claimUploadStart
       and (vh is null or vh.rental_start is null or vh.rental_start >= rentalStart)
       and (case when array_length(claimType, 1) > 0 then c.claim_type = ANY(claimType) else true end)
...
(select coalesce(avg(o.repair_gross), 0)::numeric(10,2) from claim c left outer join vehicle_hire vh on (c.vehicle_hire_id = vh.id), invoice_original o, invoice i, chorganisation cho
    where c.invoice_id = i.id and i.invoice_original_id = o.id
      and c.created_date > claimUploadStart
      and (vh is null or vh.rental_start is null or vh.rental_start >= rentalStart)
      and i.repair_net > 0
...
$$ LANGUAGE plpgsql
;
GRANT EXECUTE ON FUNCTION dlg_pilot_report(text, integer, integer[], integer[], text, text) TO chox_user;
GRANT EXECUTE ON FUNCTION dlg_pilot_report(text, integer, integer[], integer[], text, text) TO chox_mi;
