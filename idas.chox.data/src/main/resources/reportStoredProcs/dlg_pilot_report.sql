drop function dlg_pilot_report(text, integer, integer[], integer[]);
-- select * from dlg_pilot_report('2016-04-16', 6, array[1007,1008,1009,1125], null);
create or replace function dlg_pilot_report
(
   startDate text, insid integer, chorgs integer[], claimType integer[]
)
returns table
(
   id   integer,
...
from (select dat1 as startDate, chorgs as chorgIds, insid as insurerId) params
...
(select coalesce(sum(CASE WHEN i.hire_gross_paid is null THEN i.hire_gross ELSE i.hire_gross_paid END), 0)::numeric(14,2) from claim c , invoice i
    where c.invoice_id = i.id and c.status in ('PaymentReceived','ManualInvoicePaid')
      and (case when array_length(claimType, 1) > 0 then c.claim_type = ANY(claimType) else true end)
      and (c.insurer_id = params.insurerId or params.insurerId = -1)                                        
      and (c.chorganisation_id = ANY(params.chorgIds) or params.chorgIds is null)
      and i.created_date between to_date(to_char(params.startDate, 'MM') || '-01-' || to_char(params.startDate, 'yyyy'), 'mm-dd-yyyy') - interval '5 months'
...
(select coalesce(sum(io.full_total_to_pay - i.total_to_pay - i.total_penalty_charge), 0)::numeric(14,2) from claim c , invoice i, invoice_original io
    where c.invoice_id = i.id and i.invoice_original_id = io.id
      and (case when array_length(claimType, 1) > 0 then c.claim_type = ANY(claimType) else true end)
      and (c.insurer_id = params.insurerId or params.insurerId = -1)                                        
      and (c.chorganisation_id = ANY(params.chorgIds) or params.chorgIds is null)
...
(select coalesce(sum(io.full_total_to_pay - i.total_to_pay), 0)::numeric(14,2) from claim c , invoice i, invoice_original io
    where c.invoice_id = i.id and i.invoice_original_id = io.id
      and (case when array_length(claimType, 1) > 0 then c.claim_type = ANY(claimType) else true end)
      and (c.insurer_id = params.insurerId or params.insurerId = -1)                                        
      and (c.chorganisation_id = ANY(params.chorgIds) or params.chorgIds is null)
...
(select coalesce(sum(o.repair_gross - (case when i.final_payment is not null then i.repair_gross_paid else i.repair_gross end)), 0)::numeric(10,2) from claim c, invoice_original o, invoice i, chorganisation cho
   where c.invoice_id = i.id and i.invoice_original_id = o.id
     and (case when array_length(claimType, 1) > 0 then c.claim_type = ANY(claimType) else true end)
     and (c.insurer_id = params.insurerId or params.insurerId = -1)                                        
     and c.chorganisation_id = cho.id and (params.chorgIds is null or c.chorganisation_id = ANY(params.chorgIds))
...
$$ LANGUAGE plpgsql
;
GRANT EXECUTE ON FUNCTION dlg_pilot_report(text, integer, integer[], integer[]) TO chox_user;
GRANT EXECUTE ON FUNCTION dlg_pilot_report(text, integer, integer[], integer[]) TO chox_mi;
