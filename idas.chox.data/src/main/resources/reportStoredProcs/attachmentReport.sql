...
select  c.cho_reference as "Supplier Reference",
        ...
        (case when exists(select 1 from task t where t.claim_id=c.id and t.created_by = a.created_by and t.task_type='Attachment' and t.created_date between a.created_date - interval '1 second' and a.created_date + interval '1 second') then 'True' else 'False' end) as "Task Created",
