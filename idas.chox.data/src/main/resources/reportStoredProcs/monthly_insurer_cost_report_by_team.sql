...
(select avg(i.hire_gross)::numeric(8,2) from claim c , invoice i, audit_trail a, workgroup w, chorganisation cho
    where c.invoice_id = i.id
       and c.chorganisation_id = cho.id and cho.insurer_upload_only=false
       and a.claim_id = c.id
       and a.new_status = 'PaymentReceived' and c.status = 'PaymentReceived'
...
(select count(*) from claim c, invoice i, chorganisation cho, workgroup w
  where c.invoice_id=i.id
       ...
       and (i.hire_penalty_charge_paid > 0.0 or i.repair_penalty_charge_paid > 0.0)
...
(select avg(i.hire_penalty_charge_paid + i.repair_penalty_charge_paid)::numeric(8,2) from claim c, invoice i, chorganisation cho, workgroup w
  where c.invoice_id=i.id  
       and c.status = 'PaymentReceived'
       and c.workgroup_id = w.id and w.team like params.team
       and (c.insurer_id = params.insurerId or params.insurerId = -1)
...
(select sum(i.hire_penalty_charge_paid + i.repair_penalty_charge_paid)::numeric(8,2) from claim c, invoice i, chorganisation cho, workgroup w
  where c.invoice_id=i.id  
       and c.status = 'PaymentReceived'
       and c.workgroup_id = w.id and w.team like params.team
       and (c.insurer_id = params.insurerId or params.insurerId = -1)
...
UNION
 ...
 select 'no_claims_penalty_payment_charged' as title , (select count(*) from claim c, invoice i, workgroup w, chorganisation cho
   where c.invoice_id=i.id
        and c.chorganisation_id = cho.id and cho.insurer_upload_only=false
        and (c.insurer_id = params.insurerId or params.insurerId = -1)
        and (c.chorganisation_id = params.chorgId or params.chorgId = -1)
...
(select sum(i.hire_gross)::numeric(12,2) from claim c , invoice i, audit_trail a, workgroup w, chorganisation cho
    where c.invoice_id = i.id
      and c.chorganisation_id = cho.id and cho.insurer_upload_only=false
      and a.claim_id = c.id
      and a.new_status = 'PaymentReceived' and c.status = 'PaymentReceived'
