...
AS $BODY$
    ...
    BEGIN
        ...
        SELECT
            ...
            COALESCE(to_char(inv.created_date, 'DD/MM/YYYY'), '') as "Date Invoiced",
            ...
            COALESCE(to_char(vh.rental_start, 'DD/MM/YYYY'), '') as "Hire Start Date",
            COALESCE(to_char(vh.rental_end, 'DD/MM/YYYY'), '') as "Hire End Date",
            vh.days as "No. Days Hire",
            hmd.name_of_repairer as "Name of Repairer",
            COALESCE(to_char(hmd.repair_book_in_date, 'DD/MM/YYYY'), '') as "Repair Book-in Date",
            COALESCE(to_char(hmd.repair_authorised_date, 'DD/MM/YYYY'), '') as "Date Repair Authorised",
            COALESCE(to_char(hmd.repair_commenced_date, 'DD/MM/YYYY'), '') as "Date Repair Commenced",
            COALESCE(to_char(hmd.inspection_booked_date, 'DD/MM/YYYY'), '') as "Inspection Booked Date",
            COALESCE(to_char(hmd.inspection_date, 'DD/MM/YYYY'), '') as "Inspection Date",
            COALESCE(to_char(hmd.repair_completion_date, 'DD/MM/YYYY'), '') as "Repair Completion Date",
            COALESCE(to_char(hmd.total_loss_offer_made, 'DD/MM/YYYY'), '') as "Date Total Loss Offer Made",
            COALESCE(to_char(hmd.total_loss_offer_accepted, 'DD/MM/YYYY'), '') as "Date Total Loss Offer Accepted",
            COALESCE(to_char(hmd.total_loss_check_issued, 'DD/MM/YYYY'), '') as "Date Total Loss Cheque Issued",
            COALESCE(to_char(hmd.total_loss_check_received, 'DD/MM/YYYY'), '') as "Date Total Loss Cheque Received",
