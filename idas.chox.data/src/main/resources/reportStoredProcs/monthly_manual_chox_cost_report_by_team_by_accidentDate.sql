drop function manual_chox_cost_report_by_team_ad(text, int, int, text);
create or replace function manual_chox_cost_report_by_team_ad
(
...
SELECT 1 as id, 'Average Hire Days Invoiced' AS title,
  ...
  (SELECT coalesce(avg(CASE WHEN vh.days_original IS NOT NULL THEN vh.days_original ELSE vh.days END), 0)::numeric(8,1)
   FROM claim c, incident inc, invoice i, vehicle_hire vh, chorganisation cho, workgroup w
...
SELECT 3 as id, 'Average Hire Rate Invoiced' AS title,
  ...
  (SELECT coalesce(avg(CASE WHEN o IS NULL THEN i.hire_rate_charged_per_day ELSE o.hire_rate_charged_per_day END), 0)::numeric(8,2)
   FROM chorganisation cho, claim c, incident inc, workgroup w, invoice i
...
SELECT 4 as id, 'Average Hire Rate Paid' AS title,
  ...
  (SELECT coalesce(avg(i.hire_rate_charged_per_day), 0)::numeric(8,2)
   FROM claim c, incident inc, invoice i, chorganisation cho, workgroup w
   WHERE c.invoice_id = i.id
     AND c.incident_id = inc.id
     AND c.workgroup_id = w.id AND w.team like params.team
     AND i.hire_net - i.admin_fee > 0
...
SELECT 10 as id, 'Average Total Loss Hire Days Invoiced' AS title,
  ...
  (SELECT coalesce(avg(CASE WHEN vh.days_original IS NOT NULL THEN vh.days_original ELSE vh.days END), 0)::numeric(8,1)
   FROM claim c, incident inc, invoice i, vehicle_hire vh, customer cu, chorganisation cho, workgroup w
   WHERE c.invoice_id=i.id
     AND c.incident_id = inc.id
     AND c.workgroup_id = w.id AND w.team like params.team
     AND i.hire_net - i.admin_fee > 0
...
SELECT 21 as id, 'Total No. Claims Uploaded' AS title,
  ...
  (SELECT count(*)
   FROM claim c, incident inc, chorganisation cho, workgroup w
...
SELECT 22 as id, 'Total No. Invoices Paid' AS title,
  ...
  (SELECT count(*)
   FROM claim c, incident inc, audit_trail a, chorganisation cho, workgroup w
...
(select coalesce(sum(case when i.final_payment is not null then (i.hire_gross_paid + i.hire_penalty_charge_paid) else (i.hire_gross + i.hire_penalty_charge) end), 0) from claim c, incident inc, invoice i, workgroup w
    where c.invoice_id = i.id and c.status='ManualInvoicePaid'
      and c.incident_id = inc.id
      and c.workgroup_id = w.id AND w.team like params.team
      and (c.insurer_id = params.insurerId or params.insurerId = -1)                                        
...
(select coalesce(sum(io.hire_gross), 0) from claim c, incident inc, invoice i, invoice_original io, workgroup w
...
(select coalesce(sum(io.repair_gross), 0) from claim c, incident inc, invoice i, invoice_original io, workgroup w
    where c.invoice_id = i.id and i.invoice_original_id = io.id
      and c.incident_id = inc.id
      and c.workgroup_id = w.id AND w.team like params.team
      and c.claim_type IN (10,14,15,16,17)
...
(select sum(io.repair_gross) from claim c, incident inc, invoice i, invoice_original io, workgroup w
    where c.invoice_id = i.id and i.invoice_original_id = io.id
      and c.incident_id = inc.id
      and c.workgroup_id = w.id AND w.team like params.team
      and c.claim_type IN (10,14,15,16,17)
...
(select sum(CASE WHEN i.repair_gross_paid is null THEN i.repair_gross ELSE i.repair_gross_paid END) from claim c, incident inc, invoice i, workgroup w
    where c.invoice_id = i.id and c.status='ManualInvoicePaid'
      and c.incident_id = inc.id
      and c.workgroup_id = w.id AND w.team like params.team
      and c.claim_type IN (10,14,15,16,17)
...
SELECT 34 as id, 'Average Repair Duration' AS title,
  (SELECT avg(extract(epoch from (hmd.repair_completion_date - hmd.repair_commenced_date))/(3600*24))::numeric(8,1)
   FROM claim c, incident inc, invoice i, hire_monitoring_detail hmd, chorganisation cho, workgroup w
   WHERE c.invoice_id=i.id
     AND c.incident_id = inc.id
     AND c.workgroup_id = w.id AND w.team like params.team
...
GRANT EXECUTE ON FUNCTION manual_chox_cost_report_by_team_ad(text, integer, integer, text) TO chox_user;
GRANT EXECUTE ON FUNCTION manual_chox_cost_report_by_team_ad(text, integer, integer, text) TO chox_mi;
