...
CREATE OR REPLACE FUNCTION rsa_commercial_area_report(IN insid integer, IN choIds INTEGER[], IN invpaylogStartDate text, IN invpaylogEndDate text, IN claimUploadStartDate text, IN claimUploadEndDate text)
  RETURNS TABLE("CHO Reference" character varying, "Insurer Claim Number" character varying, "Original Hire Gross" numeric, "Current Hire Gross" numeric, "Hire Gross Paid" numeric, "Current Hire Penalty Charge" numeric, "Hire Penalty Charege Paid" numeric, "Days from Claim Upload Date to Payment Received" double precision, "Days from Invoice Upload Date to Payment Received" double precision, "Days from Invoice Upload Date to Invoice Payment Logged" double precision, "No. Claim Touch Points" bigint, "No. Invoice Touch Points" bigint, "Day Rate Of Hire Paid" numeric, "No. Days Hire" numeric, "Current Claim Status" character varying, "Claim Type" text, "CHO Name" character varying, "Workgroup Area" text, "Insurer Claim Owner" text, "Last Status Modified Date" timestamp without time zone, "Liability Status" text, "Date Claim Uploaded into CHOX" timestamp without time zone, "3rd Party Vehicle Reg Number" character varying) AS
$BODY$
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION rsa_commercial_area_report(integer, INTEGER[], text, text, text, text)
  OWNER TO chox;
