CREATE OR REPLACE FUNCTION choUserStateReport(IN choId integer)
RETURNS table (
   "User Name" varchar,
   "Name" text,
   "Active" text,
...
DECLARE
   cho_id INTEGER = choId::INTEGER;
   choPassChange INTEGER;
BEGIN
...
from web_user w
where chorganisation_id = cho_id
...
GRANT EXECUTE ON FUNCTION choUserStateReport(IN choId integer) TO chox_user;
GRANT EXECUTE ON FUNCTION choUserStateReport(IN choId integer) TO chox_mi;
