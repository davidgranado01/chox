...
RETURN QUERY
    ...
    select c.cho_reference, cho.name, ins.name, getClaimType(c.claim_type), c.status, to_char(i.created_date, 'dd/mm/yyyy'), vhvc.name,
           ...
           case when (repair_net_does_not_exceed_vehicle_class_repair_net_ceiling and i.repair_net <= cvcc.repair_net_ceiling and c.claim_type not in (7,8,9,11,12,13,18,19,20)) then 'Passed' else case when (repair_net_does_not_exceed_vehicle_class_repair_net_ceiling and cvcc is not null and i.repair_net > cvcc.repair_net_ceiling and c.claim_type not in (7,8,9,11,12,13,18,19,20)) then 'Failed' else 'Skipped' end end,
           ...
           case when (hire_day_count_does_not_exceed_band_hire_day_ceiling and vh.days <= bre.hire_day_ceiling and c.claim_type not in (7,8,9,18,19,20)) then 'Passed' else case when (hire_day_count_does_not_exceed_band_hire_day_ceiling and vh.days > bre.hire_day_ceiling and c.claim_type not in (7,8,9,18,19,20)) then 'Failed' else 'Skipped' end end,
           ...
           hmd.labour_rate,
           case when (maximum_labour_rate_standard_check and hmd.labour_rate is not null and cu.vehicle_class_id in (25,26,27,29,30,31,32,35,36,44,45,46,48,52,54,55,62,64,65,95,97,98,100,101,102,105,108,110,114,122,140,146,147,148,149,150,151,152,153,154,155,156,157,158,159,160,161,162,163,164,165,166,167,168,169,170,171,172,173,174,175,176,177,178,179,180,181,182,183,184,185,186,187,188,189,190,191,192,193,194,195,196,254,255,256,257,258,259,260,261,262,293,294,295,296,297,298,299,300,301,302,303,304,329,330,331,332,333,334,338,339,340,341,342,343)) then bre.max_allowed_labour_standard_rate else null end,
           case when (maximum_labour_rate_standard_check and hmd.labour_rate is not null and cu.vehicle_class_id in (25,26,27,29,30,31,32,35,36,44,45,46,48,52,54,55,62,64,65,95,97,98,100,101,102,105,108,110,114,122,140,146,147,148,149,150,151,152,153,154,155,156,157,158,159,160,161,162,163,164,165,166,167,168,169,170,171,172,173,174,175,176,177,178,179,180,181,182,183,184,185,186,187,188,189,190,191,192,193,194,195,196,254,255,256,257,258,259,260,261,262,293,294,295,296,297,298,299,300,301,302,303,304,329,330,331,332,333,334,338,339,340,341,342,343) and hmd.labour_rate <= bre.max_allowed_labour_standard_rate) then 'Passed'
