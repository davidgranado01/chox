DROP FUNCTION slaFixedFeeRejectionReportQuery(IN insurerid INTEGER, IN startdate TEXT, IN enddate TEXT, IN noRejectionDays INTEGER);

CREATE OR REPLACE FUNCTION slaFixedFeeRejectionReportQuery(IN insurerid INTEGER, IN startdate TEXT, IN enddate TEXT, IN noRejectionDays INTEGER)
RETURNS TEXT AS
        ...
        FOR i IN 1..2 LOOP
            ...
            FOR workgroupRecord IN SELECT * FROM workgroup WHERE insurer_id = $1 AND status = TRUE ORDER BY name LIMIT totalRecord LOOP
                ...
                FOR j IN 1..2 LOOP
                    ...
                    IF j=1 THEN
                       totalRejectionReasonRecord = (SELECT COUNT(*) FROM reason_of_rejection WHERE insurer_id = $1 AND type ILIKE 'Claim Rejection' AND fixed_fee_active = TRUE);
                    ELSE
                       totalRejectionReasonRecord = 1;
                    END IF;

                    FOR reasonOfRejection IN SELECT * FROM reason_of_rejection WHERE insurer_id = $1 AND type ILIKE 'Claim Rejection' AND fixed_fee_active = TRUE ORDER BY name LIMIT totalRejectionReasonRecord LOOP
                        ...
                        queryString = queryString || 'AND a.new_status = ''ClaimRejected'' ';
                        queryString = queryString || 'AND ((c.claim_type IN (11,12,13)) OR ( c.id IN (SELECT cm.claim_id FROM comment cm WHERE cm.claim_id = c.id AND cm.reverted = FALSE AND cm.comment ILIKE ''Claim switched from Fixed Fee to GTA.''))) ';
