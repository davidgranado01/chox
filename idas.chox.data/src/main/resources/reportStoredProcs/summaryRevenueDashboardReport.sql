-- select * from summary_revenue_dashboard(26, array[1007, 1123, 1341], '2016-05-02', '2016-05-08');
-- select * from summary_revenue_dashboard(26, null, '2016-05-02', '2016-05-08');
drop function summary_revenue_dashboard(IN insurerids integer, IN choids integer[], IN startdate text, IN enddate text);
create or replace function summary_revenue_dashboard
(
...
insurerChoRecord RECORD;

BEGIN 
    IF choids IS NOT NULL THEN -- If choIds provided loop through each id and also run for 'All' cho
        ...
        LOOP
            IF choids[i] IS NOT NULL THEN
                    ...
                    RETURN QUERY
                            ...
                            where
                                    ...
                                    and c.chorganisation_id IN (choids[i]::int)
                                    and c.insurer_id = insurerid
                                    and c.claim_type not in (0,1,2,6,9,13,16,20)
                                    and a.new_status in ('InvoicePaymentLogged', 'ManualInvoicePaid')
                                    and a.reverted = false
            ...
            END IF;
        END LOOP;
    ELSE -- If null is passed for choIds then run the query for each mapped cho against the given insurer and also run for 'All' cho
        FOR i IN 1..2 LOOP
            IF i=1 THEN
                FOR insurerChoRecord IN SELECT * FROM insurer_chorganisation WHERE insurer_id = insurerid LOOP
                    RETURN QUERY

                            select

                            (select name from chorganisation cho where cho.id IN (insurerChoRecord.chorganisation_id)) as "CHO Name",
                            ...
                            from 
                                    claim c
                            where 
                                    c.chorganisation_id IN (insurerChoRecord.chorganisation_id)
                                    and c.insurer_id = insurerid
                                    and c.claim_type not in (2,6,9,13,16,20)
                                    and c.created_date BETWEEN startdate::date AND enddate::date) as "Total No. Claims Uploaded",
                            ...
                            where 
                                    c.invoice_id = inv.id
                                    and c.chorganisation_id IN (insurerChoRecord.chorganisation_id)
                                    and c.insurer_id = insurerid
                                    and c.claim_type not in (2,6,9,13,16,20)
                                    and inv.created_date BETWEEN startdate::date AND enddate::date) as "Total No. Invoices Uploaded",
...
$$ LANGUAGE plpgsql
;
GRANT EXECUTE ON FUNCTION summary_revenue_dashboard(IN insurerids integer, IN choids integer[], IN startdate text, IN enddate text) TO chox_user;
GRANT EXECUTE ON FUNCTION summary_revenue_dashboard(IN insurerids integer, IN choids integer[], IN startdate text, IN enddate text) TO chox_mi;
