...
public class HandlerPerformanceReport implements Report {
    ...
    public Map<String, Object> getReportParameters() throws Exception {
        ...
        try {
            ...
            List<HandlerPerformanceReportObject> performanceReportObjects = new ArrayList<>();
            ...
            for (HandlerPerformanceReportObject obj : performanceReportObjects) {
                ...
                for (Object o : result) {
                    ...
                    sb.append("(select count(*) from claim c, audit_trail a1, audit_trail a2 where a1.created_by = :pOwnerId and c.insurer_id = :pInsurerId ")
                      ...
                      .append("and a1.reverted=false and a2.reverted=false and a2.new_status in ").append(getOutstandingStatusList()).append(" and not exists (select * from audit_trail a3 where a3.reverted=false and a3.new_status = a1.original_status and a3.update_date > a2.update_date and a3.update_date < a1.update_date and a3.claim_id=c.id) ")
                      .append(" and a1.update_date between :pStartDate and :pEndDate")
                      .append(") as taskProcessedBetweenGivenPeriod, ");

                    sb.append("(select count(*) from claim c, audit_trail a1, audit_trail a2 where a1.created_by = :pOwnerId and c.insurer_id = :pInsurerId ")
                      .append("and c.id = a1.claim_id and c.id = a2.claim_id and a2.new_status = a1.original_status and a1.update_date > a2.update_date ")
                      .append("and a1.reverted=false and a2.reverted=false and a2.new_status in ").append(getOutstandingStatusList()).append(" and not exists (select * from audit_trail a3 where a3.reverted=false and a3.new_status = a1.original_status and a3.update_date > a2.update_date and a3.update_date < a1.update_date and a3.claim_id=c.id) ")
                      .append("and a1.update_date between :pStartDate and :pEndDate")
                      .append(" and (EXTRACT(DAY FROM(a1.update_date - a2.update_date)) - COUNT_FULL_WEEKEND_DAYS(cast(a2.update_date as date), cast(a1.update_date as date))) <= 2 ")
                      .append(") as taskCompleted0_2days, ");

                    sb.append("(select count(*) from claim c, audit_trail a1, audit_trail a2 where a1.created_by = :pOwnerId and c.insurer_id = :pInsurerId ")
                      .append("and c.id = a1.claim_id and c.id = a2.claim_id and a2.new_status = a1.original_status and a1.update_date > a2.update_date ")
                      .append("and a1.reverted=false and a2.reverted=false and a2.new_status in ").append(getOutstandingStatusList()).append(" and not exists (select * from audit_trail a3 where a3.reverted=false and a3.new_status = a1.original_status and a3.update_date > a2.update_date and a3.update_date < a1.update_date and a3.claim_id=c.id) ")
                      .append(" and a1.update_date between :pStartDate and :pEndDate")
                      .append(" and (EXTRACT(DAY FROM(a1.update_date - a2.update_date)) - COUNT_FULL_WEEKEND_DAYS(cast(a2.update_date as date), cast(a1.update_date as date))) <= 5 and (EXTRACT(DAY FROM(a1.update_date - a2.update_date)) - COUNT_FULL_WEEKEND_DAYS(cast(a2.update_date as date), cast(a1.update_date as date))) > 2 ")
                      .append(") as taskCompleted2_5days, ");

                    sb.append("(select count(*) from claim c, audit_trail a1, audit_trail a2 where a1.created_by = :pOwnerId and c.insurer_id = :pInsurerId ")
                      .append("and c.id = a1.claim_id and c.id = a2.claim_id and a2.new_status = a1.original_status and a1.update_date > a2.update_date ")
                      .append("and a1.reverted=false and a2.reverted=false and a2.new_status in ").append(getOutstandingStatusList()).append(" and not exists (select * from audit_trail a3 where a3.reverted=false and a3.new_status = a1.original_status and a3.update_date > a2.update_date and a3.update_date < a1.update_date and a3.claim_id=c.id) ")
                      .append(" and a1.update_date between :pStartDate and :pEndDate")
                      .append(" and (EXTRACT(DAY FROM(a1.update_date - a2.update_date)) - COUNT_FULL_WEEKEND_DAYS(cast(a2.update_date as date), cast(a1.update_date as date))) <= 15 and (EXTRACT(DAY FROM(a1.update_date - a2.update_date)) - COUNT_FULL_WEEKEND_DAYS(cast(a2.update_date as date), cast(a1.update_date as date))) > 5 ")
                      .append(") as taskCompleted5_15days, ");

                    sb.append("(select count(*) from claim c, audit_trail a1, audit_trail a2 where a1.created_by = :pOwnerId and c.insurer_id = :pInsurerId ")
                      .append("and c.id = a1.claim_id and c.id = a2.claim_id and a2.new_status = a1.original_status and a1.update_date > a2.update_date ")
                      .append("and a1.reverted=false and a2.reverted=false and a2.new_status in ").append(getOutstandingStatusList()).append(" and not exists (select * from audit_trail a3 where a3.reverted=false and a3.new_status = a1.original_status and a3.update_date > a2.update_date and a3.update_date < a1.update_date and a3.claim_id=c.id) ")
                      .append(" and a1.update_date between :pStartDate and :pEndDate")
                      .append(" and (EXTRACT(DAY FROM(a1.update_date - a2.update_date)) - COUNT_FULL_WEEKEND_DAYS(cast(a2.update_date as date), cast(a1.update_date as date))) > 15 ")
                      .append(") as taskCompletedAfter15days, ");
                    ...
                    sb.append("(select cast(avg(total_day) as numeric(6,2)) from (select (EXTRACT(DAY FROM(a1.update_date - i.created_date))- COUNT_FULL_WEEKEND_DAYS(cast(i.created_date as date), cast(a1.update_date as date))) as total_day  from claim c, audit_trail a1, invoice i where a1.created_by = :pOwnerId and c.insurer_id = :pInsurerId ")
                      .append("and c.id = a1.claim_id and c.invoice_id = i.id ")
                      .append("and a1.reverted=false and a1.new_status in ('InvoicePaymentLogged','ManualInvoicePaid') ")
                      .append("and not exists (select * from audit_trail a2 where a2.reverted=false and a2.new_status = a1.new_status and a2.update_date < a1.update_date and c.id = a2.claim_id ) ")
                      .append("and not exists (select * from audit_trail a3 where a3.reverted=false and a3.original_status = a1.new_status and a3.new_status not in ('PaymentReceived','ClaimClosed') and c.id = a3.claim_id and a3.update_date > a1. update_date) ")
                      .append(" and a1.update_date between :pStartDate and :pEndDate")
                      .append(")  a ) as avgInvoicePaymentDay, ");
                    ...
                    sb.append("(select cast(avg(avg_day) as numeric(6,2)) from (select (EXTRACT(DAY FROM(a1.update_date - a2.update_date))- COUNT_FULL_WEEKEND_DAYS(cast(a2.update_date as date), cast(a1.update_date as date))) as avg_day from claim c, audit_trail a1, audit_trail a2 where a1.created_by = :pOwnerId and c.insurer_id = :pInsurerId ")
                      ...
                      .append("and a1.reverted=false and a2.reverted=false and a2.new_status in ").append(getOutstandingStatusList()).append(" and not exists (select * from audit_trail a3 where a3.reverted=false and a3.new_status = a1.original_status and a3.update_date > a2.update_date and a3.update_date < a1.update_date and a3.claim_id=c.id) ")
                      .append(" and a1.update_date between :pStartDate and :pEndDate")
                      .append(")  a ) as averageDaysToProcess, ");

                    sb.append("(select avg(original_total_to_pay) from (select io.full_total_to_pay as original_total_to_pay ")
                      .append("from claim c, audit_trail a1, invoice_original io, invoice i  where i.id = c.invoice_id and i.invoice_original_id=io.id and a1.created_by = :pOwnerId and c.insurer_id = :pInsurerId ")
                      .append("and c.id = a1.claim_id ")
                      .append("and a1.reverted=false and a1.new_status in ('InvoicePaymentLogged','ManualInvoicePaid') ")
                      .append("and not exists (select * from audit_trail a2 where a2.reverted=false and a2.new_status = a1.new_status and a2.update_date < a1.update_date and c.id = a2.claim_id ) ")
                      .append("and not exists (select * from audit_trail a3 where a3.reverted=false and a3.original_status = a1.new_status and a3.new_status not in ('PaymentReceived','ClaimClosed') and c.id = a3.claim_id and a3.update_date > a1. update_date) ")
                      .append("and a1.update_date between :pStartDate and :pEndDate " )
                      .append(")  h ) as originalFullTotalToPay, ");

                    sb.append("(select avg(total_to_pay) from (select i.total_to_pay as total_to_pay ")
                      .append("from claim c, audit_trail a1, invoice i  where c.invoice_id=i.id and a1.created_by = :pOwnerId and c.insurer_id = :pInsurerId ")
                      .append("and c.id = a1.claim_id ")
                      .append("and a1.reverted=false and a1.new_status in ('InvoicePaymentLogged','ManualInvoicePaid') ")
                      .append("and not exists (select * from audit_trail a2 where a2.reverted=false and a2.new_status = a1.new_status and a2.update_date < a1.update_date and c.id = a2.claim_id ) ")
                      .append("and not exists (select * from audit_trail a3 where a3.reverted=false and a3.original_status = a1.new_status and a3.new_status not in ('PaymentReceived','ClaimClosed') and c.id = a3.claim_id and a3.update_date > a1. update_date) ")
                      .append("and a1.update_date between :pStartDate and :pEndDate " )
                      .append(")  h ) as fullTotalToPay ");
