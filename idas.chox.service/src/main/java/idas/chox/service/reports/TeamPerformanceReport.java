            ...
            for (TeamPerformanceReportObject obj: teamReportObjects) {
                ...
                if (result.isEmpty())
                  ...
                  for (Object o : result) {
                    ...
                    sb.append("(select cast(avg(total_day) as numeric(6,2)) from (select (cast(a1.update_date as date) - cast(a2.update_date as date)) as total_day ");
                    ...
                    sb.append("(select cast(avg(avg_day) as numeric(6,2)) from (select (cast(a1.update_date as date) - cast(a2.update_date as date)) as avg_day ");
                    sb.append("from claim c, audit_trail a1, audit_trail a2, workgroup w  where c.workgroup_id = w.id and w.status = true ");
                    sb.append("and w.insurer_id = :pInsurerId and w.site=:pSite and w.team=:pTeam ");
                    sb.append("and c.id = a1.claim_id and c.id = a2.claim_id and a2.new_status = a1.original_status and a1.update_date > a2.update_date ");
                    sb.append("and a2.new_status in ").append(getOutstandingStatusList())
                    ...
                    sb.append("from claim c, audit_trail a1, audit_trail a2, workgroup w, invoice i  where c.invoice_id=i.id and c.workgroup_id = w.id and w.status = true ");
                    sb.append("and w.insurer_id = :pInsurerId and w.site=:pSite and w.team=:pTeam ");
                    sb.append("and c.id = a1.claim_id and c.id = a2.claim_id and a2.new_status = a1.original_status and a1.update_date > a2.update_date ");
