...
public class InvoiceSummaryReport implements Report {
    ...
    public HashMap getReportParameters() {
        ...
        try {
            ...
            sb.append("(select case when count(*) is null then 0 else count(*) end as no_count from (select case when EXTRACT(DAY FROM (audit.update_date - invoice.created_date)) is null then 0 else EXTRACT(DAY FROM (audit.update_date - invoice.created_date)) end as total_day from rpt_claim_invoice invoice inner join audit_trail audit on audit.claim_id=invoice.claim_id and audit.new_status='InvoicePaymentLogged' and not exists (select claim_id from audit_trail where status='ClaimClosed' and claim_id=audit.claim_id) and not exists (select * from audit_trail where claim_id=audit.claim_id and new_status='InvoicePaymentLogged' and update_date > audit.update_date) and status in ('InvoicePaymentLogged', 'PaymentReceived') where date(invoice.created_date) between :pInvUploadDateFrom and :pInvUploadDateTo and invoice.insurer_id=insurer_chorganisation.insurer_id and invoice.chorganisation_id=insurer_chorganisation.chorganisation_id) a where total_day <= 30) as InvoiceSettledCat0Days, ");
            sb.append("(select case when count(*) is null then 0 else count(*) end as no_count from (select case when EXTRACT(DAY FROM (audit.update_date - invoice.created_date)) is null then 0 else EXTRACT(DAY FROM (audit.update_date - invoice.created_date)) end as total_day from rpt_claim_invoice invoice inner join audit_trail audit on audit.claim_id=invoice.claim_id and audit.new_status='InvoicePaymentLogged' and not exists (select claim_id from audit_trail where status='ClaimClosed' and claim_id=audit.claim_id) and not exists (select * from audit_trail where claim_id=audit.claim_id and new_status='InvoicePaymentLogged' and update_date > audit.update_date) and status in ('InvoicePaymentLogged', 'PaymentReceived') where date(invoice.created_date) between :pInvUploadDateFrom and :pInvUploadDateTo and invoice.insurer_id=insurer_chorganisation.insurer_id and invoice.chorganisation_id=insurer_chorganisation.chorganisation_id) a where total_day > 30 and total_day <= 60) as InvoiceSettledCat30Days, ");
            sb.append("(select case when count(*) is null then 0 else count(*) end as no_count from (select case when EXTRACT(DAY FROM (audit.update_date - invoice.created_date)) is null then 0 else EXTRACT(DAY FROM (audit.update_date - invoice.created_date)) end as total_day from rpt_claim_invoice invoice inner join audit_trail audit on audit.claim_id=invoice.claim_id and audit.new_status='InvoicePaymentLogged' and not exists (select claim_id from audit_trail where status='ClaimClosed' and claim_id=audit.claim_id) and not exists (select * from audit_trail where claim_id=audit.claim_id and new_status='InvoicePaymentLogged' and update_date > audit.update_date) and status in ('InvoicePaymentLogged', 'PaymentReceived') where date(invoice.created_date) between :pInvUploadDateFrom and :pInvUploadDateTo and invoice.insurer_id=insurer_chorganisation.insurer_id and invoice.chorganisation_id=insurer_chorganisation.chorganisation_id) a where total_day > 60 and total_day <= 90) as InvoiceSettledCat60Days, ");
            sb.append("(select case when count(*) is null then 0 else count(*) end as no_count from (select case when EXTRACT(DAY FROM (audit.update_date - invoice.created_date)) is null then 0 else EXTRACT(DAY FROM (audit.update_date - invoice.created_date)) end as total_day from rpt_claim_invoice invoice inner join audit_trail audit on audit.claim_id=invoice.claim_id and audit.new_status='InvoicePaymentLogged' and not exists (select claim_id from audit_trail where status='ClaimClosed' and claim_id=audit.claim_id) and not exists (select * from audit_trail where claim_id=audit.claim_id and new_status='InvoicePaymentLogged' and update_date > audit.update_date) and status in ('InvoicePaymentLogged', 'PaymentReceived') where date(invoice.created_date) between :pInvUploadDateFrom and :pInvUploadDateTo and invoice.insurer_id=insurer_chorganisation.insurer_id and invoice.chorganisation_id=insurer_chorganisation.chorganisation_id) a where total_day > 90) as InvoiceSettledCat90Days, ");
            sb.append("(select case when count(*) is null or count(*) = 0 then 0 else cast(round(sum(EXTRACT(DAY FROM (audit.update_date - invoice.created_date)))/count(*)) as bigint) end as no_count from rpt_claim_invoice invoice inner join audit_trail audit on audit.claim_id=invoice.claim_id and audit.new_status='InvoicePaymentLogged' and not exists (select * from audit_trail where status='InvoicePaymentLogged' and update_date > audit.update_date) where date(invoice.created_date) between :pInvUploadDateFrom and :pInvUploadDateTo and invoice.insurer_id=insurer_chorganisation.insurer_id and invoice.chorganisation_id=insurer_chorganisation.chorganisation_id) as averageNoDaysOfInvoiceSettlement, ");
