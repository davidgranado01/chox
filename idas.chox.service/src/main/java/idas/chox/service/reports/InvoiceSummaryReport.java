...
public class InvoiceSummaryReport implements Report {
    ...
    public HashMap getReportParameters() {
        ...
        try {
            ...
            sb.append("(select case when sum(original_total_to_pay) is null then 0.00 else sum(original_total_to_pay) end as no_sum from rpt_claim_invoice where insurer_id = insurer_chorganisation.insurer_id and chorganisation_id = insurer_chorganisation.chorganisation_id and status!='ClaimClosed' and date_trunc('day', created_date) between :pInvUploadDateFrom and :pInvUploadDateTo) as valueInvoicesSubmitted,");
