        ...
        try {
            ...
            for (OwnerPerformanceReportObject obj : performanceReportObjects) {
                ...
                for (Object o : result) {
                    ...
                    sb.append("(select cast(avg(total_day) as numeric(6,2)) from (select ((cast(a1.update_date as date) - cast(i.created_date as date))- COUNT_FULL_WEEKEND_DAYS(cast(a1.update_date as date), cast(i.created_date as date))) as total_day  from claim c, audit_trail a1, invoice i where c.claim_owner_id = :pOwnerId ");
                    if (isWorkgroupEnabled) {
                        sb.append("and workgroup_id = :pWorkgroupId ");
                    }
