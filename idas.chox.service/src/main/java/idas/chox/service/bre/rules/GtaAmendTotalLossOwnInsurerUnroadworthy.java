...
public class GtaAmendTotalLossOwnInsurerUnroadworthy implements IBusinessRule {
    ...
    public RuleEvaluation applyToClaim(Claim claim) {
        ...
        if (claim.getBreBand().isTotalLossOwnUnroadworthyCheck() && !ClaimType.isCollaborationProtocol(claim.getClaimType())
            ...
            if (noDays1 > claim.getBreBand().getTimeToAuthoriseRepair2() || noDays2 > claim.getBreBand().getTimeToOffHire2()) {
                success = false;
                narrative = "The hire period for a total loss exceeds the number of days allowed where the claimants own insurer is dealing and the vehicle is not roadworthy.";
            }
