...
import idas.chox.service.bre.rules.MaximumLabourRate;

/**
 *
 * @author John
 */
public class Rule077MaximumLabourRateBusinessRuleTest extends BaseTest {
    ...
    private Claim getTestClaim() {
        ...
        claim.getBreBand().setMaximumLabourRateCheck(true);
        claim.getBreBand().setMaxAllowedLabourRate(BigDecimal.ZERO);
    ...
    public void testSkipped_1() throws IOException {
        ...
        RuleEvaluation rv = new MaximumLabourRate().applyToClaim(claim);

        assertTrue(RuleEvaluationResult.RULE_SKIPPED == rv.getResult());
        assertTrue(rv.getRelatedRule().getNarrative().equalsIgnoreCase(""));
        assertTrue(rv.getRelatedRule().getStatusAfterFailure(false).equals(ClaimStatus.INVOICE_ESCALATED_TO_CH));
        assertFalse(rv.getIsVisibleToCHO());
    }

    
    /*
     * Rule skipped for subsciber claims
     */
    @Test
    ...
    public void testPassed_1() throws IOException {
        ...
        claim.getBreBand().setMaxAllowedLabourRate(BigDecimal.TEN.add(new BigDecimal("0.01")));
    ...
    public void testPassed_2() throws IOException {
        Claim claim = getTestClaim();
        claim.getHireMonitoringDetail().setLabourRate(BigDecimal.TEN);
        claim.getBreBand().setMaxAllowedLabourRate(claim.getHireMonitoringDetail().getLabourRate());
    ...
    public void testFailed_1() throws IOException {
        Claim claim = getTestClaim();
        claim.getHireMonitoringDetail().setLabourRate(BigDecimal.TEN);
        claim.getBreBand().setMaxAllowedLabourRate(BigDecimal.TEN.subtract(new BigDecimal("0.01")));
