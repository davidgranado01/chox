...
import idas.chox.service.bre.rules.MaximumLabourRateStandard;

/**
 *
 * @author John
 */
public class Rule087MaximumLabourRateStandardTest extends BaseTest {
    ...
    private Claim getTestClaim() {
        ...
        claim.getBreBand().setMaximumLabourRateStandardCheck(true);
        claim.getBreBand().setMaxAllowedLabourStandardRate(BigDecimal.ZERO);
        HireMonitoringDetail hmd = new HireMonitoringDetail();
        hmd.setLabourRate(BigDecimal.ZERO);
        claim.setHireMonitoringDetail(hmd);
        claim.getCustomer().getVehicleClass().setName("S1");
        return claim;
    }
    ...
    public void testSkipped_1() throws IOException {
        ...
        claim.getBreBand().setMaximumLabourRateStandardCheck(false);
        RuleEvaluation rv = new MaximumLabourRateStandard().applyToClaim(claim);
    ...
    public void testPassed_1() throws IOException {
        ...
        claim.getBreBand().setMaxAllowedLabourStandardRate(BigDecimal.TEN.add(new BigDecimal("0.01")));
    ...
    public void testPassed_2() throws IOException {
        Claim claim = getTestClaim();
        claim.getHireMonitoringDetail().setLabourRate(BigDecimal.TEN);
        claim.getBreBand().setMaxAllowedLabourStandardRate(claim.getHireMonitoringDetail().getLabourRate());
    ...
    public void testFailed_1() throws IOException {
        Claim claim = getTestClaim();
        claim.getHireMonitoringDetail().setLabourRate(BigDecimal.TEN);
        claim.getBreBand().setMaxAllowedLabourStandardRate(BigDecimal.TEN.subtract(new BigDecimal("0.01")));
