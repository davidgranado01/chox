...
import idas.chox.service.bre.rules.CompoundAutomaticChargeCheckWithHpiLookup;
 ...
 * @author John
 */
public class Rule081CompoundAutomaticChargeCheckWithHpiLookupTest extends BaseTest {
    ...
    public void testSkipped_1() throws IOException {
        ...
        claim.getBreBand().setCompoundAutomaticChargeCheckHpiLookup(false);
        claim.getInsurer().setId(3);
        claim.getChorganisation().setId(1006);

        CompoundAutomaticChargeCheckWithHpiLookup rule = new CompoundAutomaticChargeCheckWithHpiLookup();
    ...
    public void testSkipped_2() throws IOException {
        ...
        Claim claim = getTestClaim();
        claim.getBreBand().setCompoundAutomaticChargeCheckHpiLookup(true);
    ...
    public void testSkipped_4() throws IOException {
        ...
        Claim claim = getTestClaim();
        claim.getBreBand().setCompoundAutomaticChargeCheckHpiLookup(true);
        claim.getVehicleHire().getVehicleClass().setName("CV1A");
        claim.getVehicleHire().getVehicleClass().setId(139);
        claim.getVehicleHire().setDays(1);
        claim.getVehicleHire().setHpiVehicleTransmission("We have an automatic");
    ...
    public void testFailed_1() throws IOException {
        ...
        assertTrue(rv.getRelatedRule().getNarrative().equalsIgnoreCase("The CHO is charging an automatic fee for the hire and the HPI lookup did not identify the hire vehicle to be an automatic, please review need."));
        assertTrue(ClaimStatus.INVOICE_ESCALATED_TO_CH.equals(rv.getRelatedRule().getStatusAfterFailure(claim.getClaimType())));
        assertFalse(rv.getIsVisibleToCHO());
