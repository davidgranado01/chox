...
if [ $# -ne 4  -a $# -ne 5 ]
then
    echo "Usage: $0 <insurerId> <startDate> <endDate> <directory> [xsd]"
    echo
    exit 1
fi
...
# Default to Mobility insurer if not set
: ${INS_ID:=19}
...
function generateXSD {
    ...
    if [ ! -z ${GENERATE_XSD} ]
    then
        $PSQL_COMMAND -h ${HOST} -U ${USER} -d ${DB} -o ${OUTPUT_FILE2}  << --EOF--
...
echo 'Generating a full XML data dump from '${START_DATE}' to '${END_DATE}' for insurer with id='${INS_ID}

XML_DUMPFILE_PREFIX=${DATA_DIR}/${START_DATE}

CLAIM_DATE_RESTRICTION=" and ((c.created_date >= \'"${START_DATE}"\'::Date and c.created_date < \'"${END_DATE}"\'::Date) or (c.last_modified_date >= \'"${START_DATE}"\'::Date and c.last_modified_date < \'"${END_DATE}"\'::Date))"
