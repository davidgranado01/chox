...
public class ChoDashboardBuilder {
    ...
    private String getQuery()
    {
        ...
        +"(select case when count(distinct id) is null then 0 else count(distinct id) end as no_count from claim where chorganisation_id=chorganisation.id and (insurer_id = :pInsId or :pInsId < 0) and created_date between :pSelectedStartDate and :pSelectedEndDate and status in ('ContestedInvoiceReferredToCHO','ContestedInvoiceReferredToInsurer','InvoiceApprovedByBRE','InvoiceDataCalculationIncorrect','InvoiceEscalated','InvoicePaymentLogged','AwaitingInvoicePayment','ClaimClosed','AwaitingCarHireInfo','AwaitingInvoiceData','InvoiceRejectionAccepted')) as noOfClaimNotificationsAccepted,"
        ...
        +"(select case when count(distinct id) is null then 0 else count(distinct id) end as no_count from rpt_claim_invoice where chorganisation_id=chorganisation.id and (insurer_id = :pInsId or :pInsId < 0) and created_date between :pSelectedStartDate and :pSelectedEndDate and status in ('AwaitingInvoicePayment')) as noOfInvoicesAccepted,"
        +"(select case when sum(total_to_pay-panalty_charge) is null then 0.00 else sum(total_to_pay-panalty_charge) end as no_sum from rpt_claim_invoice where chorganisation_id=chorganisation.id and (insurer_id = :pInsId or :pInsId < 0) and created_date between :pSelectedStartDate and :pSelectedEndDate and status in ('AwaitingInvoicePayment')) as valueOfInvoicesAccepted,"
        ...
        +"(select case when count(distinct id) is null then 0 else count(distinct id) end as no_count from rpt_claim_invoice where chorganisation_id=chorganisation.id and (insurer_id = :pInsId or :pInsId < 0) and created_date between :pSelectedStartDate and :pSelectedEndDate and status in ('ContestedInvoiceReferredToCHO','ContestedInvoiceReferredToInsurer','InvoiceApprovedByBRE','InvoiceDataCalculationIncorrect','InvoiceEscalated')) as noOfInvoicesPending,"
        +"(select case when sum(total_to_pay-panalty_charge) is null then 0.00 else sum(total_to_pay-panalty_charge) end as no_sum from rpt_claim_invoice where chorganisation_id=chorganisation.id and (insurer_id = :pInsId or :pInsId < 0) and created_date between :pSelectedStartDate and :pSelectedEndDate and status in ('ContestedInvoiceReferredToCHO','ContestedInvoiceReferredToInsurer','InvoiceApprovedByBRE','InvoiceDataCalculationIncorrect','InvoiceEscalated')) as valueOfInvoicesPending,"
        ...
        +"(select case when count(distinct id) is null then 0 else count(distinct id) end as no_count from rpt_claim_invoice where chorganisation_id=chorganisation.id and (insurer_id = :pInsId or :pInsId < 0) and created_date between :pSelectedStartDate and :pSelectedEndDate and status in ('InvoicePaymentLogged')) as noOfInvoicesPaymentLogged,"
        +"(select case when sum(total_to_pay-panalty_charge) is null then 0.00 else sum(total_to_pay-panalty_charge) end as no_sum from rpt_claim_invoice where chorganisation_id=chorganisation.id and (insurer_id = :pInsId or :pInsId < 0) and created_date between :pSelectedStartDate and :pSelectedEndDate and status in ('InvoicePaymentLogged')) as valueOfInvoicesPaymentLogged,"
