...
public class ChoDashboardBuilder {
    ...
    private String getQuery()
    {
        ...
        */ 
        ...
        /* 
        + "(select case when count(distinct a.claim_id) is null then 0 else count(distinct a.claim_id) end as no_count from rpt_claim_audit_trail a, rpt_claim_invoice b WHERE a.chorganisation_id=chorganisation.id and (a.insurer_id = :pInsId or :pInsId < 0) AND a.claim_id=b.claim_id AND date(a.update_date) BETWEEN :pSelectedStartDate and :pSelectedEndDate AND a.new_status in ('AwaitingInvoicePayment')) as noOfInvoicesAccepted,"
        + "(select case when sum(b.total_to_pay-b.panalty_charge) is null then 0.00 else sum(b.total_to_pay-b.panalty_charge) end as no_sum from rpt_claim_audit_trail a, rpt_claim_invoice b WHERE a.chorganisation_id=chorganisation.id and (a.insurer_id = :pInsId or :pInsId < 0) AND a.claim_id=b.claim_id AND date(a.update_date) BETWEEN :pSelectedStartDate and :pSelectedEndDate AND a.new_status in ('AwaitingInvoicePayment')) as valueOfInvoicesAccepted,"
        ...
        + "(select case when count(distinct a.claim_id) is null then 0 else count(distinct a.claim_id) end as no_count from rpt_claim_audit_trail a, rpt_claim_invoice b WHERE a.chorganisation_id=chorganisation.id and (a.insurer_id = :pInsId or :pInsId < 0) AND a.claim_id=b.claim_id AND date(a.update_date) BETWEEN :pSelectedStartDate and :pSelectedEndDate AND a.new_status in ('ContestedInvoiceReferredToCHO','ContestedInvoiceReferredToInsurer','InvoiceApprovedByBRE','InvoiceDataCalculationIncorrect','InvoiceEscalated','ClaimPending', 'InvoiceReferredToClaimHandler')) as noOfInvoicesPending,"
        + "(select case when sum(b.total_to_pay-b.panalty_charge) is null then 0.00 else sum(b.total_to_pay-b.panalty_charge) end as no_sum from rpt_claim_audit_trail a, rpt_claim_invoice b WHERE a.chorganisation_id=chorganisation.id and (a.insurer_id = :pInsId or :pInsId < 0) AND a.claim_id=b.claim_id AND date(a.update_date) BETWEEN :pSelectedStartDate and :pSelectedEndDate AND a.new_status in ('ContestedInvoiceReferredToCHO','ContestedInvoiceReferredToInsurer','InvoiceApprovedByBRE','InvoiceDataCalculationIncorrect','InvoiceEscalated','ClaimPending', 'InvoiceReferredToClaimHandler')) as valueOfInvoicesPending,"
