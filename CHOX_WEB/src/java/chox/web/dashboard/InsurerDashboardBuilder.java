...
public class InsurerDashboardBuilder {
    ...
    private String getQuery()
    {
        ...
        + "(select case when count(distinct a.id) is null then 0 else count(distinct a.id) end as no_count from rpt_claim_audit_trail a, rpt_claim_invoice b WHERE (a.chorganisation_id = :pChorganisationId or :pChorganisationId < 0) AND a.insurer_id=insurer.id AND a.claim_id=b.claim_id AND date(a.update_date) BETWEEN :pSelectedStartDate and :pSelectedEndDate AND a.new_status in ('AwaitingInvoicePayment')) as noOfInvoicesAccepted,"
        + "(select case when sum(b.total_to_pay-b.panalty_charge) is null then 0.00 else sum(b.total_to_pay-b.panalty_charge) end as no_sum from rpt_claim_audit_trail a, rpt_claim_invoice b WHERE (a.chorganisation_id = :pChorganisationId or :pChorganisationId < 0) AND a.insurer_id=insurer.id AND a.claim_id=b.claim_id AND date(a.update_date) BETWEEN :pSelectedStartDate and :pSelectedEndDate AND a.new_status in ('AwaitingInvoicePayment')) as valueOfInvoicesAccepted,"
