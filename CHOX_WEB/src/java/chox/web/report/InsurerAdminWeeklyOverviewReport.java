        ...
        try {
            ...
            do {
                ...
                */
                ...
                + "(select case when count(distinct id) is null then 0 else count(distinct id) end as no_count from rpt_claim_audit_trail a, (select claim_id, max(update_date) as max_update_date from rpt_claim_audit_trail where update_date between :pSelectedStartDate and :pSelectedEndDate group by claim_id) b where a.insurer_id=insurer.id AND a.update_date between :pSelectedStartDate and :pSelectedEndDate AND a.claim_id=b.claim_id AND a.update_date=b.max_update_date AND a.new_status in ('ContestedInvoiceReferredToCHO','InvoiceDataCalculationIncorrect')) as contestedinvoiceByRsa, "
                ...
                + "0 as paidInvoicePercentage, "
                + "(select case when count(distinct claim_id) is null then 0 else count(distinct claim_id) end as no_count from rpt_claim_audit_trail where insurer_id = insurer.id and update_date between :pSelectedStartDate and :pSelectedEndDate and new_status='ClaimUnacknowledgedUnrouted' and claim_id not in (select case when count(distinct claim_id) is null then 0 else count(distinct claim_id) end as no_count from audit_trail where new_status in ('InvoiceApprovedByBRE','InvoiceDataCalculationIncorrect','InvoiceEscalated') and update_date between :pSelectedStartDate and :pSelectedEndDate)) as claimTobeInvoiced "
                + "from insurer insurer where insurer.id = :pInsId";
